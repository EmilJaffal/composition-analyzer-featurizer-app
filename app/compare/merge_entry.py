...
def combine_features_with_database_excel(script_dir_path):
    ...
    cif_set_feat, featurized_sheet_name = excel.load_data_from_excel(featurized_file_path)
...
def merge_excel_data(
    ...
    df1 = df1.rename(columns={c: normalize[c.strip().lower()] for c in df1.columns if c.strip().lower() in normalize})
    df2 = df2.rename(columns={c: normalize[c.strip().lower()] for c in df2.columns if c.strip().lower() in normalize})

    # Ensure merge key exists in both
    if 'Entry' not in df1.columns or 'Entry' not in df2.columns:
        raise KeyError("Missing 'Entry' column in one of the datasets after normalization.")
